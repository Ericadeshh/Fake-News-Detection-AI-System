...
import { FaReact, FaSearch, FaBrain, FaShieldAlt, FaCode, FaDatabase, } from "react-icons/fa";
    ...
    const features = [
        {
            ...
            details: "The system processes various input types through Flask API endpoints. Text is analyzed directly, files are parsed using PyPDF2 and python-docx, and URLs are scraped using BeautifulSoup.",
        },
        {
            icon: <FaBrain />,
            title: "AI Detection Engine",
            description: "LSTM neural network with TF-IDF feature extraction",
            details: "Uses a custom LSTM model trained on 400k+ articles. Includes SMOTE for class balancing, advanced text cleaning, and feature engineering. Achieves 98.6% accuracy with continuous learning from user feedback.",
        },
        {
            icon: <FaShieldAlt />,
            title: "Verification Pipeline",
            description: "Full-stack validation system with feedback loop",
            details: "Implements content validation, error handling, and user feedback integration. Stores analysis history in SQL database with confidence scores and user corrections.",
        },
    ];
    return (<motion.div initial={{ opacity: 0 }} animate={{ opacity: isMounted ? 1 : 0 }} transition={{ duration: 0.5 }} className={styles.aboutSection}>
      ...
      <div className={styles.overview}>
        ...
        <motion.p className={styles.sectionText} initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }}>
      ...
      <div className={styles.howItWorks}>
        ...
        <div className="row g-4">
          {features.map((feature, index) => (<div key={index} className="col-lg-4 col-md-6 col-12">
              <motion.div className={styles.featureCard} whileHover={{ y: -5 }} layout>
                <div className={styles.featureIcon}>{feature.icon}</div>
                <h4 className={styles.featureTitle}>{feature.title}</h4>
                <p className={styles.featureText}>{feature.description}</p>

                <motion.div className={styles.readMore} onClick={() => setExpandedSection(expandedSection === index ? null : index)} whileHover={{ scale: 1.05 }}>
                  <AiOutlineExpand />{" "}
                  {expandedSection === index ? "Show Less" : "Read More"}
                </motion.div>

                <AnimatePresence>
                  {expandedSection === index && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className={styles.expandedContent}>
                      <p>{feature.details}</p>
      ...
      <motion.div className={styles.credits} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
