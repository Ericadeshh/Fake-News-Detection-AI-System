...
import { FaCheck, FaTimes, FaRedo, FaArrowLeft, FaExclamationTriangle, FaInfoCircle, } from "react-icons/fa";
import styles from "./AnalysisResult.module.css";
const AnalysisResult = ({ result, resetForm, submitFeedback, feedbackStatus, changeFeedbackAnalysis, error, setError, }) => {
    if (!result)
        return null;
    return (<>
      <motion.div className={`${styles.resultCard} ${result.prediction === "true" ? styles.true : styles.fake}`} initial={{ scale: 0.95 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 300 }}>
        <div className={styles.resultHeader}>
          <h2>
            Analysis Result: <span>{result.prediction.toUpperCase()}</span>
          </h2>
          <motion.button onClick={resetForm} className={styles.newAnalysisButton} whileHover={{ x: -3 }}>
      ...
      <AnimatePresence>
        {error && (<motion.div className={styles.errorAlert} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <div>
